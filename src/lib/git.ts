 ...
 */
export async function getGitCommitDates(repoUrl: string): Promise<GitCommitDates> {
  try {
 ...
 */
async function fetchGitHubCommitDates(repoPath: string): Promise<GitCommitDates> {
  try {
    ...
    const firstCommit = allCommits[allCommits.length - 1]?.commit?.author?.date || lastCommit;

    return {
      firstCommit: firstCommit ? new Date(firstCommit).toISOString().split('T')[0] : null,
      lastCommit: lastCommit ? new Date(lastCommit).toISOString().split('T')[0] : null,
    };
  } catch (error) {
